...
class PUQUncertaintyRegion:
    ...
    @staticmethod
    def _compute_pcs_masks(svs, lambda1s):
        K = svs.shape[1]
        all_energies = []
        for i in range(K):
    ...
    def compute_coverage_loss(svs, lower, upper, projected_ground_truths_minus_mean, lambda1s, lambda2s):
        pcs_masks = PUQUncertaintyRegion._compute_pcs_masks(svs, lambda1s)
    ...
    def compute_reconstruction_loss(pcs, svs, ground_truths_minus_mean, projected_ground_truths_minus_mean, lambdas_list, pixel_ratio=0.9):
        pcs_masks = PUQUncertaintyRegion._compute_pcs_masks(svs, lambdas_list)
    ...
    def calibration(self, samples_dataset: DiffusionSamplesDataset, ground_truths_dataset: GroundTruthsDataset, verbose=True):
        ...
        if valid_indices.shape[0] == 0:
            misc.handdle_error(self._calibration_failed_message(samples_dataset.num_images))
    ...
    def inference(self, samples):
        ...
        mu, pcs, svs, lower, upper = self._compute_intervals(samples)
        mu, pcs, svs, lower, upper = self._apply_lambdas(
            mu, pcs, svs, lower, upper)

        if isinstance(pcs, list):
            pcs = [t.cpu() for t in pcs]
            svs = [t.cpu() for t in svs]
            lower = [t.cpu() for t in lower]
            upper = [t.cpu() for t in upper]
            return mu.cpu(), pcs, svs, lower, upper
        else:
            return mu.cpu(), pcs.cpu(), svs.cpu(), lower.cpu(), upper.cpu()
...
class EPUQUncertaintyRegion(PUQUncertaintyRegion):
    ...
    def _compute_batch_losses(self, pcs, svs, lower, upper, ground_truths_minus_mean, projected_ground_truths_minus_mean):
        ...
        coverage_losses = PUQUncertaintyRegion.compute_coverage_loss(
    ...
    def _assign_lambdas(self, valid_indices, verbose=True):
        ...
        if verbose:
            logging.info(f'Successfully calibrated: lambda={lamb:.2f}')
...
class DAPUQUncertaintyRegion(PUQUncertaintyRegion):
    ...
    def _compute_batch_losses(self, pcs, svs, lower, upper, ground_truths_minus_mean, projected_ground_truths_minus_mean):
        ...
        reconstruction_losses = PUQUncertaintyRegion.compute_reconstruction_loss(
    ...
    def _assign_lambdas(self, valid_indices, verbose=True):
        ...
        if verbose:
            logging.info(f'Successfully calibrated: lambda1={lambda1:.2f}, lambda2={lambda2:.2f}')

        self.reconstruction_lambda = lambda1.item()
        self.coverage_lambda = lambda2.item()

    def _apply_lambdas(self, mu, pcs, svs, lower, upper):
        pcs_masks = PUQUncertaintyRegion._compute_pcs_masks(
...
class RDAPUQUncertaintyRegion(DAPUQUncertaintyRegion):
    ...
    def _compute_losses(self, samples_dataset: DiffusionSamplesDataset, ground_truths_dataset: GroundTruthsDataset, verbose=True):
        ...
        Ks = RDAPUQUncertaintyRegion._lambda3_to_K(lambda3s, samples_dataset.num_samples_per_image)
    ...
    def _assign_lambdas(self, valid_indices, verbose=True):
        ...
        if verbose:
            logging.info(
                f'Successfully calibrated: lambda1={lambda1:.2f}, lambda2={lambda2:.2f}, lambda3={lambda3:.2f}')

        self.reconstruction_lambda = lambda1.item()
        self.coverage_lambda = lambda2.item()
        self.max_pcs = RDAPUQUncertaintyRegion._lambda3_to_K(lambda3, self.max_pcs)
